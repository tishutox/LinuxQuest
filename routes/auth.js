...
router.post('/report/:username', (req, res) => {
  try {
    ...
    // User kann sich selbst nicht melden
...
router.get('/admin/reports/:username', (req, res) => {
  try {
    ...
    if (!isProtectedUser(requester)) {
      return res.status(403).json({ error: 'Nur Admins können Reports sehen.' });
    }
    ...
    return res.status(500).json({ error: 'Serverfehler beim Abrufen der Reports.' });
  }
});
